...
def TSplot(idxAc, Time, effNS, MaxInterI, TimeShift, T, R, runname, run, SAVE):
    ...
    fig12.line(Time[idxAc], MaxInterI[:,R,T], line_width=1.5, line_color="black")
    fig12.yaxis.axis_label = 'Max Intercorrelation'
    ...
    if SAVE == 1:
        reset_output()
        name = '../Results_wAmp/'+runname+'_'+run+'_timeshift_unFilt_T'+str(T+1)+'R'+str(R+1)
...
def loadResFullrun2(resultsRun):
    file = sio.loadmat(resultsRun)
    # for chanName in file.keys():
    #     data[chanName] = file[chanName]
