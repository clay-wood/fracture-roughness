...
def plotter(data, x, y, y2=None, y3=None, idx1=0, idx2=-1, dec=1, plot_type="xy", ylog=False, y2log=False, y3log=False):
    ...
    # Plotting Settings:
    # -------------------------------------------------------------------------------------------------------- 
    color1 = '#016450'  # Greenish
    color2 = '#cc4c02' # Orange/Brown
    color3 = '#41b6c4' # LightBlue
    color4 = '#fed976'
    textsize1 = 16
    textsize2 = 24
    ...
    # --------------------------------------------------------------------------------------------------------
        ...
        if x != None:
            ...
            axs.plot(data[y2][idx1:idx2:dec], color=color1, label=y2)
            axs.set_ylabel(y, **axis_font)
            axs.set_xlabel("record #", **axis_font)
...
def plotfr(data, x, y, Fs, idx1, idx2, maglog=False, showphase=False):
    ...
    ax1.plot(data[x][idx1:idx2], data[y][idx1:idx2], color="mediumblue")
    ...
    if maglog != True:
        ...
        ax2.magnitude_spectrum(data[y][idx1:idx2], Fs=Fs, color=color1)
    else:
        ax2.set_title("Log. Magnitude Spectrum")
        ax2.magnitude_spectrum(data[y][idx1:idx2], Fs=Fs, scale='dB', color=color1)

    if showphase == True:
        ax3 = fig.add_subplot(313, sharex=ax2)
        ax3.set_title("Phase Spectrum ")
        ax3.phase_spectrum(data[y][idx1:idx2], Fs=Fs, color="crimson")
